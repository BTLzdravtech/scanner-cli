...
module.exports = class RC {
  constructor () {
    ...
    this.overcommit = false
  }

  withOvercommit (overcommit = true) {
    this.overcommit = overcommit
    if (this.target !== undefined || this.all || this.staged) {
      throw new Error('Overcommit flag (-o, --overcommit) and Target flag (-t, --target) or Staged flag (-g, --staged) or All flag (-a, --all) are mutually exclusive!')
    }
  }
  ...
  checkRc () {
    const rcFile = this.overcommit ? '.hawkeyerc' : path.join(this.target, '.hawkeyerc')
    const rcExclude = this.overcommit ? '.hawkeyeignore' : path.join(this.target, '.hawkeyeignore')
  ...
  isExcluded (code) {
    return this.exclude.reduce((result, pattern) => result || pattern.test(code), false)
  }
}
