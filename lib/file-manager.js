...
module.exports = class FileManager {
  constructor ({ overcommit, overcommitFiles, target, staged, all, exclude = [] }) {
    this.exclude = exclude
    this.files = {}
    if (overcommit) {
      this.overcommit = overcommit
      overcommitFiles.forEach(file => {
        this.addFile(file)
      })
    } else {
      this.target = target
  ...
  readFileSync (file) {
    ...
    const absolute = this.overcommit ? file : path.join(this.target, file)
